...
- The forum post ["Spine-Unity 4.2 to 4.3 Upgrade Guide"](https://esotericsoftware.com/forum/d/29234-spine-unity-42-to-43-upgrade-guide) for a comprehensive spine-csharp migration guide
...
3. **✏️ TEST AND UPDATE YOUR CODE**
   ...
   - **Test:** Open a few test scenes to see which component references are lost. Unsaved scenes still contain the old component data and serve as a safety backup - only when you save will the split components replace the old ones and references be lost.
     **⚠️ PREFAB NOTE:** Selecting a prefab asset will temporarily show upgraded components in the Inspector, but it does not save these modifications to the prefab immediately. Prefabs are persisted only when saved in Prefab Mode (`Auto Save` on or manual save). When saving a scene with prefab instances, modifications and upgrades are saved as scene overrides as usual.
   - **Decide:** Either manually re-assign lost references, or write migration code (explained below) to handle this automatically

   **⚠️ IMPORTANT:** Automatic component splitting is only persisted when scenes/prefabs are loaded and then saved, or when you use `Upgrade All`. You must choose `Upgrade All` or save each scene/prefab before building your game; otherwise old single components may remain in closed assets and required split components may be missing in the build.
...
**In short:** In 4.3, `SkeletonAnimation` and `SkeletonMecanim` are now responsible only for animation-related functionality, while rendering-related functionality is handled by `SkeletonRenderer` or `SkeletonGraphic`. `SkeletonGraphic` no longer covers anything animation-related, and `SkeletonAnimation` and `SkeletonMecanim` no longer cover rendering.
...
- **SkeletonAnimation no longer covers rendering, it is now a separate component from SkeletonRenderer**, no longer a subclass.
...
#### 1. Component Architecture
- **SkeletonMecanim no longer covers rendering, it is now a separate component from SkeletonRenderer**, no longer a subclass.
...
Once you are confident that all Spine scenes and prefabs have been upgraded and saved, you can disable automatic split component upgrade:

1. Go to `Edit → Preferences → Spine`
2. Under `Automatic Component Upgrade`, click `Split Component Upgrade` → `Disable`

This helps to:
* reveal scenes or prefabs which were not yet upgraded, as missing split components will no longer be added automatically and you will get error messages in the Console window.
* improve editor performance by stopping upgrade checks when scenes or prefabs are loaded.

If you find missing components, re-enable `Split Component Upgrade` and upgrade the assets as usual by opening, verifying, and saving the problematic assets.
