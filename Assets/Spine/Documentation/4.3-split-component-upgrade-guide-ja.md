...
- フォーラム投稿 ["Spine-Unity 4.2 to 4.3 Upgrade Guide"](https://ja.esotericsoftware.com/forum/d/29254-spine-unity-42-%E3%81%8B%E3%82%89-43-%E3%81%B8%E3%81%AE%E3%82%A2%E3%83%83%E3%83%97%E3%82%B0%E3%83%AC%E3%83%BC%E3%83%89%E3%82%AC%E3%82%A4%E3%83%89) (こちらは包括的なspine-csharp移行ガイドです)
...
3. **✏️ コードのテストと更新を行う**
   ...
   - **テストを行う：** いくつかのテストシーンを開いてみて、どのコンポーネント参照が失われるかを確認してください。未保存のシーンには古いコンポーネントデータが含まれており、セーフティバックアップとして機能します。保存すると、分離されたコンポーネントが古いものを置き換え、参照が失われます。  
     **⚠️ プレハブに関する注意：** プレハブアセットを選択すると、Inspectorにアップグレードされたコンポーネントが一時的に表示されますが、これらの変更はすぐにプレハブに保存されるわけではありません。プレハブの変更内容は、Prefab Modeで保存された場合（`Auto Save` がオンになっているか、手動で保存した場合）にのみ反映されます。プレハブインスタンスを含むシーンを保存する場合、変更やアップグレードは通常通り、シーンのオーバーライドとして保存されます。
   - **対応を決める：** 失われた参照を手動で再割り当てするか、移行コードを記述して自動的に処理してください(詳しくは後述)。

   **⚠️ 重要な注意事項：** コンポーネントの自動分割は、シーンやプレハブをロードしてから保存した場合か `Upgrade All` を使用した場合にのみ反映されます。つまり、ゲームをビルドする前に、`Upgrade All` を選択するか、各シーンやプレハブを保存する必要があります。そうしないと、閉じたアセット内に古い単一のコンポーネントが残ったままになったり、ビルドに必要な分割済みコンポーネントが欠落したりする可能性があるので注意してください。
...
**要約すると:** 4.3では、`SkeletonAnimation` および `SkeletonMecanim` はアニメーション関連の機能のみを担当するようになり、レンダリング関連の機能は `SkeletonRenderer` または `SkeletonGraphic` によって処理されるようになりました。 同時に、`SkeletonGraphic` はアニメーション関連の機能を一切扱わなくなり、`SkeletonAnimation` および `SkeletonMecanim` はレンダリングを扱わなくなりました。
...
#### 1. コンポーネントアーキテクチャ
- **SkeletonAnimation はレンダリング機能を扱わなくなり、SkeletonRenderer とは別のコンポーネントになりました**。もはやそのサブクラスではありません。
...
#### 1. コンポーネントアーキテクチャ
- **SkeletonMecanim はレンダリング機能を扱わなくなり、SkeletonRenderer とは別のコンポーネントになりました**。もはやそのサブクラスではありません。
...
すべてのSpineシーンとプレハブがアップグレードされ、保存されたことを確認したら、コンポーネントの自動分割アップグレードを無効にすることができます：

1. `Edit → Preferences → Spine` に移動
2. `Automatic Component Upgrade` の下で、`Split Component Upgrade` → `Disable` をクリック

これにより、以下の効果が得られます：
* まだアップグレードされていないシーンやプレハブを特定できるようになります。分割コンポーネントが欠落している場合、それらが自動的に追加されなくなり、コンソールウィンドウにエラーメッセージが表示されるためです。
* シーンやプレハブのロード時にアップグレードチェックが行われなくなるため、エディターのパフォーマンスが向上します。

コンポーネントの欠落が見つかった場合は、`Split Component Upgrade` を再度有効にし、問題のあるアセットを開いて内容を確認し、保存することで、通常通りアセットをアップグレードしてください。
