#if UNITY_EDITOR
#pragma warning disable 0618 // vHierarchy still relies on Unity editor APIs that are obsolete in Unity 6 but required for older-version compatibility.
...
namespace VHierarchy
{
    public static class VHierarchy
    {
        static void GameObjectRowGUI(GameObject go, Rect rowRect)
        {
            ...
            void setState()
            {
                void set_isRowSelected()
                {
                    ...
                    var dragSelectionList = GetDragSelectionInstanceIds();
        ...
        static List<int> GetDragSelectionInstanceIds()
        {
#if UNITY_2021_1_OR_NEWER
            var dragSelection = treeViewController?.GetFieldValue("m_DragSelection", false);
            var rawList = dragSelection?.GetFieldValue("m_List", false);
#else
            var rawList = treeViewController?.GetFieldValue("m_DragSelection", false);
#endif

            return ToInstanceIds(rawList);

        }

        static List<int> ToInstanceIds(object rawIds)
        {
            if (rawIds == null) return null;

            if (TryGetInstanceId(rawIds, out var singleId))
                return new List<int> { singleId };

            if (!(rawIds is IEnumerable ids)) return null;

            var result = new List<int>();

            foreach (var id in ids)
                if (TryGetInstanceId(id, out var instanceId))
        ...
        static bool TryGetInstanceId(object value, out int instanceId)
        {
            instanceId = 0;
            ...
            if (TryGetEntityIdObject(value) is Object unityObject)
            {
            ...
            try
            {
                ...
                {
                    instanceId = System.Convert.ToInt32(value);
                    return true;
                }
            }
            catch { }
        ...
        static Object TryGetEntityIdObject(object value)
        {
            var valueType = value.GetType();

            if (valueType.FullName != "UnityEngine.EntityId") return null;

            entityIdToObjectMethodInfo ??= typeof(EditorUtility).GetMethod("EntityIdToObject", BindingFlags.Public | BindingFlags.Static, null, new[] { valueType }, null);

            return entityIdToObjectMethodInfo?.Invoke(null, new[] { value }) as Object;

        }
        static MethodInfo entityIdToObjectMethodInfo;
        ...
        {
            var rawExpandedIds = hierarchyWindow?.GetFieldValue("m_SceneHierarchy")?.GetFieldValue("m_TreeViewState")?.GetPropertyValue("expandedIDs", false);

            expandedIds = ToInstanceIds(rawExpandedIds) ?? new List<int>();
